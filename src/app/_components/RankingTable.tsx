...
type RankingTableProps<T> = Readonly<{
  ...
  extra?: (ranking: CompetitionRank<T>) => React.ReactNode;
...
export default function RankingTable<T>({
  ...
  return (
    <table className="w-full">
      <tbody>
        {displayRows.map((r) => {
          const extraJsx = extra != null ? extra(r) : null;
          return (
            <tr className="whitespace-nowrap" key={keyFn(r.item)}>
              ...
              {extraJsx != null ? (
                <td
                  className={clsx(
                    "w-10 pl-2",
                    highlight(r) && `${highlightStyles} rounded-r-md`,
                  )}
                >
                  {extraJsx}
                </td>
